...
# pbmc_path <- paste0("../data2/", start_dim, "_", end_dim, "/pbmc_pca.rds")
...
if (gene_length > 0)
{
	...
	for (i in 1:gene_length)
	{
		...
		# {
			# gene_dims <- tsnes[cells_id, ]
		
			# cur_x <- paste0("x", j)
			# cur_y <- paste0("y", j)

			# cur_dim <- subset(gene_dims, select=c(cur_x, cur_y))
			...
			# x <- densCols(cur_dim[[cur_x]], cur_dim[[cur_y]], colramp=colorRampPalette(c("black", "white")))
			# dens <- col2rgb(x)[1,] + 1L
			
			
			# Q <- quantile(dens, probs=c(.25, .75), na.rm = FALSE)
			# iqr <- IQR(dens)
